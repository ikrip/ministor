...
const RegisterPage = observer(({ onGoToLogin, onSuccess }: Props) => {
  const { authStore } = useStores();
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');

  const handleSubmit = async () => {
    const ok = await authStore.register(email, password);
    if (ok) onSuccess();
  };
  ...
  return (
    <div style={styles.wrapper}>
      <div style={styles.card}>
        ...
        <p style={styles.subtitle}>Создайте аккаунт редактора</p>
